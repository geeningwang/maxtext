...
class MaxEngine(_BaseEngine):
  ...
  def load_params(self, *args, params=None, rng: PRNGKeyType | None = None, **kwargs) -> Params:
    ...
    if self.prefill_kv_cache_annotations is not None:
    ...
    if self.kv_cache_annotations is not None:
  ...
  def _generate_jit(
    ...
    raw_cache = new_vars.get("cache", decode_state.get("cache", {}))
    new_cache = jax.lax.with_sharding_constraint(raw_cache, self.kv_cache_shardings) if self.kv_cache_shardings is not None else raw_cache
    # sampling tokens
    rng, new_rng = jax.random.split(rng)
